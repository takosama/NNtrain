...
internal static class ArcTrainingMath
{
    ...
    private static float ClipResident(IEnumerable<Parameter> parameters, float maxNorm)
    {
        ...
        for (int i = 0; i < tensors.Length; i++)
        {
            ...
            ReduceToSlot(tensor.ArcGradient(), tensor.Numel, sums, i, squared: true);
        }
        ...
        foreach (Tensor tensor in tensors) lane.Run("resident_clip_apply", tensor.Numel, 0, tensor.ArcGradient(), result, tensor.Numel);
        var host = new float[3]; lane.Read(result, host);
        if (host[2] != 0 || !float.IsFinite(host[0])) throw new ArithmeticException("Arc gradient or BFP8 publication is non-finite; refusing the optimizer update.");
        return host[0];
    }

    private static void ReduceToSlot(ArcBuffer input, int n, ArcBuffer output, int slot, bool squared)
    {
        var lane = Tensor.ArcLane;
        ...
        try
        {
            do
            {
                ...
                lane.Run("reduce_sum", groups * 256L, 256, input, partials, n, squared ? 1 : 0, new LocalMemory(1024));
    ...
    internal static bool GradientsFinite(IEnumerable<Parameter> parameters)
    {
        ...
        foreach (Parameter parameter in parameters)
            if (parameter.T.HasGradientBuffer)
                lane.Run("resident_finite", parameter.T.Numel, 0, parameter.T.ArcGradient(), status, parameter.T.Numel);
