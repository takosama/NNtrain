...
internal sealed class CudaBFloat16GradientAllReducePlan : IDisposable
{
        ...
        IReadOnlyList<int> devices)
    {
        ...
        _buckets = BuildBuckets(_parameters);
    ...
    internal void Complete()
    {
        ...
        if (!_overlapExchange)
        {
            // On WDDM/consumer pairs without peer access, cudaMemcpyPeerAsync
            // stages through host memory. Running that traffic concurrently
            // with GEMM slows both down; retain BF16 packing but defer the
            // exchange until backward has completely populated all buckets.
            for (int bucket = 0; bucket < _buckets.Length; ++bucket)
    ...
    private DeviceBuffers CreateDeviceBuffers(int deviceSlot)
    {
        ...
        try
        {
            for (int bucket = 0; bucket < _buckets.Length; bucket++)
            {
                ...
                buckets[bucket] = new BucketBuffers(
                    accelerator.Allocate1D<ushort>(length),
                    accelerator.Allocate1D<ushort>(length),
                    new NativeCudaArena<float>(accelerator, length),
                    CudaGradientBuckets.CreateReadyEvent(
                        accelerator, deviceIndex));
            }
    ...
    private static Bucket[] BuildBuckets(IReadOnlyList<Parameter> parameters)
    {
