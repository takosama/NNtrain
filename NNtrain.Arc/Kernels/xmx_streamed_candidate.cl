...
#define XMX_STREAMED(NAME,ROW_BLOCKS) \
...
__kernel void NAME(__global const ushort* a,__global const uint* b,__global float* c, \
 ...
 int row=get_group_id(1)*(16*ROW_BLOCKS*8)+sg*(ROW_BLOCKS*8),colBase=get_group_id(0)*32; \
 int rows=(m+7)/8,cols=(n+15)/16,end=min(k,kStart+kCount); \
 float8 sums[ROW_BLOCKS][2]; \
 _Pragma("unroll") \
 ...
 for(int rb=0;rb<ROW_BLOCKS;rb++){ \
  _Pragma("unroll") \
  for(int tile=0;tile<2;tile++){int col=colBase+tile*16+lane; \
   _Pragma("unroll") \
   for(int r=0;r<8;r++)if(row+rb*8+r<m&&col<n)c[(row+rb*8+r)*n+col]=relu?fmax(0.0f,sums[rb][tile][r]):sums[rb][tile][r]; \
  } \
 } \
}
XMX_STREAMED(gemm_xmx_streamed_block_8x32_wg16,1)
XMX_STREAMED(gemm_xmx_streamed_block_16x32_wg16,2)
#undef XMX_STREAMED
#endif
