// Parallel split-K workspace kernel. Each K slice owns a disjoint output.
float split_bf16(float x){uint u=as_uint(x);if((u&0x7f800000u)==0x7f800000u)return x;return as_float((u+0x7fffu+((u>>16)&1u))&0xffff0000u);}
__attribute__((reqd_work_group_size(16,16,1)))
__kernel void gemm_split(__global const float* a,__global const float* b,__global float* c,
 ...
 kStart+=(int)get_group_id(2)*kCount;
 c+=(int)get_group_id(2)*m*n;
 ...
 for(int base=kStart;base<end;base+=16){
  ...
  for(int part=0;part<4;part++){
   ...
   if(r<m&&q<end){int idx=ta?q*m+r:r*k+q;av=gateOperand==1&&gate[idx]<=0?0:a[idx];if(bf16Operands&1)av=split_bf16(av);}
   at[ar+16*part][ak]=av;
   int col=cb+bc+16*part,s=base+bk;float bv=0;
   if(col<n&&s<end){int idx=tb?col*k+s:s*n+col;bv=gateOperand==2&&gate[idx]<=0?0:b[idx];if(bf16Operands&2)bv=split_bf16(bv);}
   bt[bk][bc+16*part]=bv;
  }
...
__kernel void gemm_split_finish(__global const float* partials,__global float* c,int length,int groups){
 int i=get_global_id(0);if(i>=length)return;float sum=c[i];
 for(int group=0;group<groups;group++)sum+=partials[group*length+i];
 c[i]=sum;
}
