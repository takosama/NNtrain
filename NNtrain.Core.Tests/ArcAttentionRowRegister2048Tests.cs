...
public sealed class ArcAttentionRowRegister2048Tests
{
    [Fact]
    public void CandidatePreservesReferenceBitsForFreshSavedAndBackward()
    {
        Assert.SkipWhen(!Tensor.IsArcAvailable(), "Intel Arc is required.");
        ArcDeviceInfo device = ArcDevices.Enumerate()[0];
        Assert.SkipWhen(!device.SupportsXmx || device.MinimumSubgroupSize != 16,
            "The SG16 candidate requires an Intel XMX device with 16-wide subgroups.");

        using var lane = new ArcExecutionLane();
        // Two heads also cover the first/head offset in the saved-statistics ABI.
        ...
        foreach (int causal in new[] { 0, 1, 2 })
        {
            ...
            Snapshot Run(string forward, string backward)
            {
                ...
                lane.Run(forward, rows * 64L, 64, recomputed, statistics,
                ...
                var p = new float[elements]; var savedP = new float[elements];
                var dp = new float[elements];
                var stats = new float[(first + count + 1) * sequence * 2];
                lane.Read(probability, p); lane.Read(recomputed, savedP);
                lane.Read(derivative, dp); lane.Read(statistics, stats);
                AssertBits(p, savedP, $"fresh/saved, causal={causal}");
                return new(p, savedP, dp, stats);
            }

            Snapshot expected = Run("attention_probabilities", "attention_derivatives");
            Snapshot actual = Run("attention_probabilities_register_2048",
                "attention_derivatives_register_2048");
            Snapshot fixedShape = Run("attention_probabilities", "attention_derivatives_fixed_2048");
            Snapshot prefix = Run("attention_probabilities", "attention_derivatives_prefix_2048");
            Snapshot prefix4 = Run("attention_probabilities", "attention_derivatives_prefix4_2048");
            Snapshot prefix4Subgroup = Run("attention_probabilities", "attention_derivatives_prefix4_sg16_2048");
            Snapshot prefix8 = Run("attention_probabilities", "attention_derivatives_prefix8_2048");
            Snapshot prefix16 = Run("attention_probabilities", "attention_derivatives_prefix16_2048");
            ...
            AssertBits(expected.Derivatives, fixedShape.Derivatives, $"fixed-shape derivative, causal={causal}");
            AssertBits(expected.Derivatives, prefix.Derivatives, $"prefix derivative, causal={causal}");
            AssertBits(expected.Derivatives, prefix4.Derivatives, $"prefix4 derivative, causal={causal}");
            AssertBits(expected.Derivatives, prefix4Subgroup.Derivatives, $"prefix4 SG16 derivative, causal={causal}");
            AssertBits(expected.Derivatives, prefix8.Derivatives, $"prefix8 derivative, causal={causal}");
            AssertBits(expected.Derivatives, prefix16.Derivatives, $"prefix16 derivative, causal={causal}");
        }
    }
