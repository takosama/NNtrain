...
public sealed class ArcDkvPartialUnroll2048CandidateTests
{
    [Fact]
    public void T2048CausalD32PartialUnrollPreservesBitsAcrossHeadBoundaryAndTwoAccumulations()
    {
        ...
        const int batch = 2, heads = 3, dimension = 32, sequence = 2048;
        const int width = heads * dimension, first = 2, count = 2;
        // The two consecutive heads cross from batch 0 to batch 1.
        ...
        for (int key = 0; key < sequence; ++key)
        {
            ...
            // The masked portion inside the diagonal key tile is zero.
        }
        ...
        float[] Run(string kernel)
        {
            ...
            lane.Synchronize();
            Assert.Equal(h2dBefore, lane.H2DBytes);
            Assert.Equal(d2hBefore, lane.D2HBytes);
        ...
        Assert.NotEqual(BitConverter.SingleToInt32Bits(initial[width + (first % heads) * dimension]),
            BitConverter.SingleToInt32Bits(expected[width + (first % heads) * dimension]));
        var expectedBits = MemoryMarshal.Cast<float, int>(expected.AsSpan());
        foreach (string kernel in new[]
        {
            "attention_dkv_block_slm_2048_unroll4_causal",
            "attention_dkv_block_slm_2048_unroll8_causal",
            "attention_dkv_block_slm_2048_unroll16_causal"
        })
        {
            ...
            for (int i = 0; i < expectedBits.Length; ++i)
                if (expectedBits[i] != actualBits[i])
                    Assert.Fail($"{kernel} differs from the production block-SLM kernel at element {i}: "
